...
static reflectCssClass(a,b){document.querySelectorAll("form[ui-action=\""+a+"\"] [type=\"submit\"], form[ui-action=\""+a+"\"] button:not([type]), [ui-state=\""+a+"\"]").forEach(a=>l.setCssClass(a,b))}static get options(){return n}static set options(a){b(n,a)}// #endregion
     ...
     */class p{constructor(a){this.options={baseUrl:null,types:null,keys:{data:null,type:"type",id:"id"},per_page:0,query:{"page[number]":"page[number]","page[size]":"page[size]"},fetch:{method:"GET",headers:new Headers({"content-type":"application/json"}),mode:"no-cors"}},b(this.options,a||{});var c=new k;// Accept an array of store keys
if(c._paging={},this.options.types instanceof Array){var d={};// Turn them into seperate stores
for(var e in this.options.types)d[this.options.types[e]]=this.options.types[e];this.options.types=d}if(this.options.types instanceof Object)for(var f in this.options.types)c[this.options.types[f]]=new k;this._cache=new k(c)}data(a){return this.options.keys.data?a[this.options.keys.data]:a}type(a){var b=a[this.options.keys.type];return this.options.types[b]&&(b=this.options.types[b]),b}id(a){return a[this.options.keys.id]}sync(a){var c=this.data(a);if(!c)throw new Error("Store: No data to sync");if(c instanceof Array){var f={};for(var g in c){var h=null;this.options.keys.data?(h={},h[this.options.keys.data]=c[g]):h=c[g],f[this.id(c[g])]=this.sync(h)}return f}var d=this.type(c),e=this.id(c);return this._cache[d]||(this._cache[d]=new k),this._cache[d][e]||(this._cache[d][e]=new k),b(this._cache[d][e],c),this._cache[d][e]}url(a){var b=this.options.types[a.type]||a.type,c=this.options.baseUrl+"/"+b+(a.id?"/"+a.id:""),d=[];for(var f in a)f in this.options.query?d.push(this.options.query[f]+"="+a[f]):"type"!=f&&"id"!=f&&d.push(f+"="+a[f]);var e=d.join("&");return e&&(c+="?"+e),c}search(a){var b=new k;for(var e in this._cache)if(!("_paging"==e||a.type&&this.type({type:a.type})!=e))for(var f in this._cache[e]){var c=!0,d=this.data(this._cache[e][f]);for(var g in a)if("type"!=g&&(!d.hasOwnProperty(g)||d[g]!=a[g])){c=!1;break}c&&(b[f]=d)}return b}fetch(a,b){var c=2<arguments.length&&void 0!==arguments[2]?arguments[2]:{};if(a=this.type({type:a}),"object"==typeof b&&(c=b,b=c.id),this._cache[a]){if(null!=b&&this._cache[a][b])return Promise.resolve(this._cache[a][b]);if(null==b&&0==Object.keys(c).length)return Promise.resolve(this._cache[a]);if(null==b){c.type=a;var e=this.search(c);if(Object.keys(e).length)return Promise.resolve(e)}}Object.assign(c,{type:a,id:b});var d=this.url(c);return fetch(d,this.options.fetch).then(a=>a.json()).then(a=>this.sync(a))}page(a){var b=1<arguments.length&&void 0!==arguments[1]?arguments[1]:1,c=2<arguments.length&&void 0!==arguments[2]?arguments[2]:0;c=parseInt(c)||this.options.per_page,b=parseInt(b)||1;var d=this._cache._paging;if(a in d&&c in d[a]&&b in d[a][c])return Promise.resolve().then(()=>d[a][c][b]);a in d||(d[a]={}),c in d[a]||(d[a][c]={}),b in d[a][c]||(d[a][c][b]=new k({type:a,number:b,size:c,count:0,links:{},model:new k}));var e=this.url({type:a,"page[number]":b,"page[size]":c});return fetch(e,this.options.fetch).then(a=>a.json()).then(e=>d[a][c][b].sync({links:e.links||{},count:Object.keys(this.data(e)).length,model:this.sync(e)}))}post(a,b){var c=Object.create(this.options.fetch);c.method="POST",a=a||this.type(b);var d=this.url({type:a,id:this.id(b)});return fetch(d,c).then(a=>a.json()).then(a=>this.sync(a))}patch(a,b){var c=Object.create(this.options.fetch);c.method="PATCH",a=a||this.type(b);var d=this.url({type:a,id:this.id(b)});return fetch(d,c).then(a=>a.json()).then(a=>this.sync(a))}delete(a,b){var c=Object.create(this.options.fetch);c.method="DELETE",a=a||this.type(data);var d=this.url({type:a,id:b});return fetch(d,c).then(()=>delete this._cache[a][b])}}/**
     * View
     ...
     */class q{constructor(a,b,c){c instanceof k||(c=new k(c||{})),this._name=a,this._html=b,this._model=c,c.watch(a=>this.update(a)),this.constructor.options.autoCache&&q.cache(this)}clear(){return document.querySelectorAll("[ui-view=\""+this._name+"\"]").forEach(a=>{a.innerHTML=""}),this}// When the model updates
...
static cache(a){if(this.options.verbose&&console.info(this.name+":cache()",{view:a}),"string"==typeof a)return r[a];if(a instanceof q){if(a.name in r)throw"View name already exists: \""+a.name+"\"";return r[a.name]=a,this}return r}// Render all views
static get options(){return s}static set options(a){b(s,a)}// #endregion
}var r={},s={verbose:!1,autoCache:!0,// Automatically cache views when created
eventRender:new CustomEvent("view.render",{bubbles:!0,detail:{type:"render",view:null}})};/**
     ...
     */class t extends q{constructor(a,b,c){2==arguments.length&&(c=b,b=void 0),b||(b=a),super(a,null,c),this.file=b}clear(){return this.html=null,super.clear()}render(){this.constructor.options.verbose&&console.info(this.constructor.name+".render()",this.name,{view:this});var a=null==this._html?this.fetch():Promise.resolve();return a.then(()=>super.render())}fetch(){return this.constructor.options.verbose&&console.info(this.constructor.name+".fetch()",this.name,{view:this}),fetch(this.fullPath).then(a=>{if(a.ok)return a.text();throw new Error(a.statusText)}).then(a=>this._html=a).catch(()=>{throw new Error("File not found: "+this.fileName)})}get fileName(){return this.file+"."+this.constructor.options.extension}get fullPath(){return this.constructor.options.basePath+this.fileName}get file(){return this._file}set file(a){a!=this._file&&(this._html=null),this._file=a}// #region Static methods
// View factory
static create(a){return this.options.verbose&&console.info(this.name+":create()",{options:a}),a.file&&(a.file=a.name),new this(a.name,a.file,a.model)}static get options(){return u}static set options(a){b(u,a)}// #endregion
}var u={basePath:"view/",extension:"html"};t.options=q.options;/**
     * ViewHandlebars
     * @description View that uses Handlebars templates instead of html
     * @update Support for pre-compiled templates
     */class v extends t{render(){this.constructor.options.verbose&&console.info(this.constructor.name+".render()",this.name,{view:this});var a=a=Promise.resolve();return null==this.html&&Handlebars.templates&&Handlebars.templates[this.file]&&(this.html=Handlebars.templates[this.file]),a.then(()=>super.render())}fetch(){return super.fetch().then(a=>("templates"in Handlebars||(Handlebars.templates=[]),Handlebars.templates[this.file]=Handlebars.compile(a),this.html=Handlebars.templates[this.file],a))}// #region Static methods
static get options(){return w}static set options(a){b(w,a)}// #endregion
}var w={};v.options=t.options,v.options.extension="hbs";class x{static start(){window.addEventListener("popstate",this.onPopState.bind(this)),z.interceptLinks&&document.addEventListener("click",this.onClick.bind(this)),this.navigate(location.pathname)}static navigate(a){var c=1<arguments.length&&void 0!==arguments[1]?arguments[1]:{};z.verbose&&console.info("Router.navigate()",{router:this,route:a,data:c});var d=this.sanitizePath(a),f=d[0],g=d[1],h=this.controllerName(f),i={data:c};this.controllers[h]||(f=z.defaultController,h=this.controllerName(f),g="/"==a?z.defaultMethod:d[0]),y[h]||(this.controllers[h]?y[h]=new this.controllers[h](this.view):y[h]=new o(this.view)),g=g||z.defaultMethod;var j=null,k=f+z.pathSeparator+g,l=e(g);y[h].view instanceof t?y[h].view.file=k:y[h].view instanceof v&&(y[h].view.html=k),z.autoload&&!(y[h][l]instanceof Function)&&(l=z.autoloadMethod),i.dev=location.host==z.devHost,i.view=k,i.controller=f,i.method=g,i.path=Array.from(d);// Query string to object
var m=location.search.substring(1);m&&(m=JSON.parse("{\""+decodeURI(m).replace(/"/g,"\\\"").replace(/&/g,"\",\"").replace(/=/g,"\":\"")+"\"}"),i.search=m);try{var n=Array.from(d);n.shift(),n.shift(),n.push(m),n.push(c),y[h].view.model.clear().sync(i).clearChanges(),j=y[h][l].apply(y[h],n)}catch(a){j=this.handleError(h,i,d)}if(location.pathname!=a&&(b(this.state,{route:a,controller:f,method:g,data:Object.assign({},c),search:m}),history.pushState(this.state,null,a)),j instanceof Promise){var p=document.querySelectorAll("[ui-view=\""+this.view.name+"\"]");this.handleLoading(p,!0),j.then(()=>this.handleLoading(p,!1)).catch(a=>{throw this.handleLoading(p,!1),this.handleError(h,i,d,a),a})}return j}static handleLoading(a,b){return b?a.forEach(a=>a.classList.add(z.cssClass.loading)):a.forEach(a=>a.classList.remove(z.cssClass.loading)),this}static handleError(a,b,c,d){return b.error=d,b.view=b.controller+z.pathSeparator+z.errorView,b.path=c.join(z.pathSeparator),y[a].view instanceof t?y[a].view.file=(z.useControllerErrorViews?b.controller+z.pathSeparator:"")+z.errorView:y[a].view.html="Error loading "+b.path,y[a].view.model.clear().sync(b).clearChanges(),y[a][z.errorMethod]()}static sanitizePath(a){for(var b in z.verbose&&console.info("Router.sanitizePath()",{router:this,path:a}),"string"==typeof a&&(a=a.split("/"),a.shift(),a[0]=a[0]||z.defaultController),a)a[b]=(a[b]+"").toLowerCase();return 1==a.length&&a.push(z.defaultMethod),a}static controllerName(a){return z.verbose&&console.info("Router.controllerName()",{router:this,name:a}),d(e(a))+"Controller"}static onClick(a){"A"==a.target.tagName&&a.target.pathname&&(a.preventDefault(),this.navigate(a.target.pathname,Object.assign({},a.target.dataset)))}static onPopState(a){z.verbose&&console.info("Router.changeState()",{router:this,event:a});var b=location.pathname,c={};a.state&&(b=a.state.route,c=a.state.data),this.navigate(b,c)}// #region Properties
static get controllers(){return this.options.controllers}static set controllers(a){this.options.controllers=a}static get state(){return this.options.state}static set state(a){this.options.state=a}static get view(){return this.options.view instanceof q||(this.view=this.options.view),this.options.view}static set view(a){this.options.view=a instanceof q?a:new t(a)}static get options(){return z}static set options(a){b(z,a)}// #endregion
}var y={},z={controllers:{},view:"controller",state:{},autoload:!0,verbose:!1,cssClass:{loading:"loading",success:"success",fail:"fail"},defaultController:"default",autoloadMethod:"__autoload",defaultMethod:"index",errorMethod:"__error",errorView:"error",useControllerErrorViews:!1,devHost:"localhost",interceptLinks:!0,pathSeparator:"/"};return a.Action=l,a.Controller=o,a.JsonApi=class extends k{constructor(a,b){// Accept an array of store keys
...
var c=b;for(var d in b=new k,c)b[c[d]]=new k}b._paging={},super(b),this.baseUrl=a}sync(a){if(!a.hasOwnProperty("data"))throw new Error("JsonApi: No data to sync");if(a.data instanceof Array){var e={};for(var f in a.data)e[a.data[f].id]=this.sync({data:a.data[f]});return e;//this[json.data[0].type]
            ...
            */}},a.Model=k,a.Router=x,a.Store=p,a.StoreJsonApi=class extends p{constructor(a){var c={keys:{data:"data",type:"type",id:"id"},query:{"page[number]":"page[number]","page[size]":"page[size]"},per_page:0};b(c,a||{}),super(c)}},a.StoreWordpress=class extends p{constructor(a){var c={baseUrl:"/wp-json/wp/v2",types:{page:"pages",post:"posts",category:"categories",tag:"tags",comment:"comments"},keys:{data:null,type:"type",id:"id"},query:{"page[number]":"page","page[size]":"per_page"},per_page:10};b(c,a||{}),super(c)}fetch(a,b){var c=2<arguments.length&&void 0!==arguments[2]?arguments[2]:{};return"string"==typeof b&&(c.slug=b,b=void 0),super.fetch(a,b,c)}},a.Util=g,a.View=q,a.ViewFile=t,a.ViewHandlebars=v,a}({});