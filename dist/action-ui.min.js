"use strict";var ActionUI=function(a){'use strict';/**
     ...
     */class k{constructor(){var a=0<arguments.length&&void 0!==arguments[0]?arguments[0]:{};g.set(this,{}),h.set(this,[]),j.set(this,null);var c=new Proxy(this,{get:(a,b)=>"watch"==b||"clearChanges"==b?function(){return a[b].apply(a,arguments)}:Reflect.get(a,b),set:(a,b,c)=>{var d=a[b];return Reflect.set(a,b,c),a._change(a,b,c,d),!0},deleteProperty:(a,b)=>{if(b in a){var c=a[b];delete a[b],a._change(a,b,void 0,c)}return!0}});return b(c,a),c}clear(){for(var a in this)delete this[a];return this}// Sync data object to model
sync(a){return a=Object.assign({},a),b(this,a),this}// Add watcher callback
watch(a){return h.get(this).push(a),this}clearChanges(){return g.set(this,{}),this}// Trigger all watcher callbacks
_trigger(){window.clearTimeout(j.get(this)),j.set(this,null);var a=g.get(this);if(0<Object.keys(a).length){//console.log('changes',changes,this)
var b=h.get(this);for(var c in b)b[c].call(this,a);// Clear change list
g.set(this,{})}}// Log property change
_change(a,b,c,d){g.get(a)[b]={value:c,originalValue:d},j.get(a)||j.set(a,window.setTimeout(()=>a._trigger(),10))}}/**
     * Action UI
...
static init(){document.addEventListener("click",a=>{if(a.target.matches("[ui-action]")&&"FORM"!=a.target.tagName){var b=a.target.getAttribute("ui-action");// Don't run the action if it's already running
if(b in m)!1==m[b].running&&m[b].run(a.target);else if(n.autoCreate){// Auto create and run action
var c=l.createFromElement(a.target);l.cache(c),c.run(a.target)}else throw new Error("Action not found: "+b)}}),document.addEventListener("submit",a=>{if(a.target.matches("form[ui-action]")){a.preventDefault();var b=a.target.getAttribute("ui-action");// Don't run the action if it's already running
if(b in m)!1==m[b].running&&m[b].run(a.target);else if(n.autoCreate){// Auto create and run action
...
static reflectCssClass(a,b){document.querySelectorAll("form[ui-action=\""+a+"\"] [type=\"submit\"], form[ui-action=\""+a+"\"] button:not([type]), [ui-state=\""+a+"\"]").forEach(a=>l.setCssClass(a,b))}static get options(){return n}static set options(a){b(n,a)}// #endregion
     ...
     */class p{constructor(a,b,c){c instanceof k||(c=new k(c||{})),this._name=a,this._html=b,this._model=c,c.watch(a=>this.update(a)),r.autoCache&&p.cache(this)}// When the model updates
update(a){r.verbose&&console.info("View.update()",this.name,{view:this,changes:a}),this.render()}// Render the model data into the view template for all elements with [ui-view=this.name]
/**
         * Render view to all matching containers
         * @returns Promise
         */render(){return r.verbose&&console.info("View.render()",this.name,{view:this}),document.querySelectorAll("[ui-view=\""+this._name+"\"]").forEach(a=>{a.innerHTML=this.html,this.renderSubviews(a)}),Object.assign(r.eventRender.detail,{view:this}),document.dispatchEvent(r.eventRender),Promise.resolve()}renderSubviews(a){r.verbose&&console.info("View.renderSubviews()",this.name,{view:this,parent:a}),a.querySelectorAll("[ui-view]").forEach(a=>{var b=a.getAttribute("ui-view"),c=this.__proto__.constructor.cache(b);c&&c.render()})}get name(){return this._name}get html(){return this._html instanceof Function?this._html(this._model):this._html}set html(a){this._html=a}get model(){return this._model}// #region Static methods
// View factory
static create(a){return new p(a.name,a.html,a.model)}// Cache a view 
static cache(a){if("string"==typeof a)return q[a];if(a.name in q)throw"View name already exists: \""+a.name+"\"";return q[a.name]=a,p}// Render all views
static get options(){return r}static set options(a){b(r,a)}// #endregion
}var q={},r={verbose:!1,autoCache:!1,// Automatically cache views when created
eventRender:new CustomEvent("view.render",{bubbles:!0,detail:{type:"render",view:null}})};/**
     ...
     */class s extends p{constructor(a,b,c){2==arguments.length&&(c=b,b=void 0),b||(b=a),super(a,null,c),this.file=b}render(){t.verbose&&console.info("ViewFile.render()",this.name,{view:this});var a=null==this._html?this.fetch():Promise.resolve();return a.then(()=>super.render())}fetch(){return fetch(this.fullPath).then(a=>{if(a.ok)return a.text();throw new Error(a.statusText)}).then(a=>this._html=a).catch(()=>{throw new Error("File not found: "+this.fileName)})}get fileName(){return this.file+"."+this.__proto__.constructor.options.extension}get fullPath(){return this.__proto__.constructor.options.basePath+this.fileName}get file(){return this._file}set file(a){a!=this._file&&(this._html=null),this._file=a}// #region Static methods
// View factory
static create(a){return a.file&&(a.file=a.name),new s(a.name,a.file,a.model)}static get options(){return t}static set options(a){b(t,a)}// #endregion
}var t={basePath:"",extension:"html"};s.options=p.options;/**
     * ViewHandlebars
     * @description View that uses Handlebars templates instead of html
     * @update Support for pre-compiled templates
     */class u extends s{render(){v.verbose&&console.info("ViewHandlebars.render()",this.name,{view:this});var a=null;return null==this._html?Handlebars.templates&&Handlebars.templates[this.file]?(this._html=Handlebars.templates[this.file],a=Promise.resolve()):a=this.fetch().then(()=>this._html=Handlebars.compile(this._html)):a=Promise.resolve(),a.then(()=>super.render())}// #region Static methods
static get options(){return v}static set options(a){b(v,a)}// #endregion
}var v={};u.options=s.options,u.options.extension="hbs";class w{constructor(){var a=0<arguments.length&&void 0!==arguments[0]?arguments[0]:{},b=1<arguments.length&&void 0!==arguments[1]?arguments[1]:"controller",c=2<arguments.length&&void 0!==arguments[2]?arguments[2]:{};this.controllers=a,this._instance={},this.view=b instanceof p?b:new s(b),this.state=c,window.addEventListener("popstate",this.changeState.bind(this))}navigate(a){var c=1<arguments.length&&void 0!==arguments[1]?arguments[1]:{};x.verbose&&console.info("Router.navigate()",{router:this,route:a,data:c});var d=this.sanitizePath(a),f=d[0],g=d[1],h=this.controllerName(f),i={data:c};this.controllers[h]||(f=x.defaultController,h=this.controllerName(f),g="/"==a?x.defaultMethod:d[0]),this._instance[h]||(this.controllers[h]?this._instance[h]=new this.controllers[h](this.view):this._instance[h]=new o(this.view)),g=g||x.defaultMethod;var j=null,k=f+"/"+g,l=e(g);this._instance[h].view instanceof s?this._instance[h].view.file=k:this._instance[h].view instanceof u&&(this._instance[h].view.html=k),x.autoload&&!(this._instance[h][l]instanceof Function)&&(l=x.autoloadMethod),i.dev=location.host==x.devHost,i.view=k,i.controller=f,i.method=g;// Query string to object
var m=location.search.substring(1);m&&(m=JSON.parse("{\""+decodeURI(m).replace(/"/g,"\\\"").replace(/&/g,"\",\"").replace(/=/g,"\":\"")+"\"}"),i.search=m);try{var n=Array.from(d);n.shift(),n.shift(),n.push(m),n.push(c),this._instance[h].view.model.clear().sync(i).clearChanges(),j=this._instance[h][l].apply(this._instance[h],n)}catch(a){j=this.handleError(h,i,d)}if(location.pathname!=a&&(b(this.state,{route:a,controller:f,method:g,data:Object.assign({},c),search:m}),history.pushState(this.state,null,a)),j instanceof Promise){var p=document.querySelectorAll("[ui-view=\""+this.view.name+"\"]");this.handleLoading(p,!0),j.then(()=>this.handleLoading(p,!1)).catch(a=>{throw this.handleLoading(p,!1),this.handleError(h,i,d,a),a})}return j}handleLoading(a,b){return b?a.forEach(a=>a.classList.add(x.cssClass.loading)):a.forEach(a=>a.classList.remove(x.cssClass.loading)),this}handleError(a,b,c,d){return this._instance[a].view instanceof s?this._instance[a].view.file=x.errorView:this._instance[a].view instanceof u&&(this._instance[a].view.html=x.errorView),b.error=d,b.view=x.errorView,b.path=c.join("/"),this._instance[a].view.model.clear().sync(b).clearChanges(),this._instance[a][x.errorMethod]()}sanitizePath(a){for(var b in x.verbose&&console.info("Router.sanitizePath()",{router:this,path:a}),"string"==typeof a&&(a=a.split("/"),a.shift(),a[0]=a[0]||x.defaultController),a)a[b]=(a[b]+"").toLowerCase();return 1==a.length&&a.push(x.defaultMethod),a}controllerName(a){return x.verbose&&console.info("Router.controllerName()",{router:this,name:a}),d(e(a))+"Controller"}changeState(a){x.verbose&&console.info("Router.changeState()",{router:this,event:a});var b=location.pathname,c={};a.state&&(b=a.state.route,c=a.state.data),this.navigate(b,c)}// #region Static methods
static get options(){return x}static set options(a){b(x,a)}// #endregion
}var x={autoload:!0,verbose:!1,cssClass:{loading:"loading",success:"success",fail:"fail"},defaultController:"default",autoloadMethod:"__autoload",defaultMethod:"index",errorMethod:"__error",errorView:"error",devHost:"localhost"};return a.Action=l,a.Controller=o,a.JsonApi=class extends k{constructor(a,b){// Accept an array of store keys
...
var c=b;for(var d in b=new k,c)b[c[d]]=new k}b._paging={},super(b),this.baseUrl=a}sync(a){if(!a.hasOwnProperty("data"))throw new Error("JsonApi: No data to sync");if(a.data instanceof Array){var e={};for(var f in a.data)e[a.data[f].id]=this.sync({data:a.data[f]});return e;//this[json.data[0].type]
            ...
            */}},a.Model=k,a.Router=w,a.Util=f,a.View=p,a.ViewFile=s,a.ViewHandlebars=u,a}({});