...
static init(){document.addEventListener("click",t=>{let e=s(t.target,"[ui-action]"),o=t.target.disabled||t.target.hasAttribute("disabled");if(e&&(o=o||e.disabled||e.hasAttribute("disabled")),e&&"FORM"!=e.tagName&&!o){"INPUT"==e.tagName&&("checkbox"==e.type||"radio"==e.type)||t.preventDefault();var n=e.getAttribute("ui-action");// Don't run the action if it's already running
if(n in y)!1==y[n].running&&y[n].run(e);else if(g.autoCreate){// Auto create and run action
var a=c.createFromElement(e);c.cache(a),a.run(e)}else throw new Error("Action not found: "+n)}}),document.addEventListener("submit",t=>{if(t.target.matches("form[ui-action]")&&!t.target.disabled&&!t.target.hasAttribute("disabled")){t.preventDefault();var e=t.target.getAttribute("ui-action");// Don't run the action if it's already running
