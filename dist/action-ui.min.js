...
return c._options={triggerDelay:10},c._changes={},c._watchers=[],c._timer=null,c._parent=b,c.sync(a),c.clearChanges(),c}clear(){for(let a in this)try{delete this[a]}catch(b){// Fallback for IE11
let c=this[a];this[a]=void 0,this._change(a,void 0,c)}return this}// Sync data object to model
...
// View factory
static create(a){return this.options.verbose&&console.info(this.name+":create()",{options:a}),a.file&&(a.file=a.name),new this(a.name,a.file,a.model)}static setCssClass(a,b){for(var c in w.cssClass)a.classList.remove(w.cssClass[c]);a.classList.add(b)}static get options(){return w}static set options(a){b(w,a)}// #endregion
